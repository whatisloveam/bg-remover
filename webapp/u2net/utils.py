...
class RescaleT(object):
    ...
    def __call__(self, sample):
        ...
        # #resize the image to new_h x new_w and convert 
        # image from range [0,255] to [0,1]
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        # resize the image to new_h x new_w and 
        # convert image from range [0,255] to [0,1]
        img = transform.resize(image, (new_h, new_w), mode="constant")
        lbl = transform.resize(
            label, (new_h, new_w), mode="constant", 
            order=0, preserve_range=True
        )
...
class ToTensorLab(object):
    ...
    def __call__(self, sample):
        ...
        if self.flag == 2:  # with rgb and Lab colors
            ...
            for i in range(0, 6):
                tmp = tmpImg[:, :, i]-np.mean(tmpImg[:, :, i])
                tmpImg[:, :, i] = tmp/np.std(tmpImg[:, :, ])
...
class SalObjDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # #io.imread(self.image_name_list[idx])
        # label = Image.open(self.label_name_list[idx])
        # #io.imread(self.label_name_list[idx])
